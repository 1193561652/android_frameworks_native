...
std::vector<compositionengine::LayerFE::LayerSettings> EffectLayer::prepareClientCompositionList(
    ...
    std::optional<compositionengine::LayerFE::LayerSettings> layerSettings = prepareClientComposition(targetSettings);
    // Nothing to render.
    if (!layerSettings) {
        return {};
    }
    ...
    if (targetSettings.realContentIsVisible && fillsColor()) {
        ...
        results.push_back(*layerSettings);
    } else if (getIndex()>0.0_hf && getIndex() < 1.0_hf) {
        results.push_back(*layerSettings);
    }
...
bool EffectLayer::isVisible() const {
    bool bret = !isHiddenByPolicy() && getAlpha() > 0.0_hf && hasSomethingToDraw();
    // ALOGE("BAT %p EffectLayer::isVisible ret:%d %d %d %d", this, bret, isHiddenByPolicy(), getAlpha() > 0.0_hf, hasSomethingToDraw());
    return bret;
}
