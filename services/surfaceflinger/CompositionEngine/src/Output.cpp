...
void Output::cacheClientCompositionRequests(uint32_t cacheSize) {
    ALOGE("BAT Output::cacheClientCompositionRequests");
    if (cacheSize == 0) {
...
void Output::ensureOutputLayerIfVisible(sp<compositionengine::LayerFE>& layerFE,
    ...
    if (CC_UNLIKELY(!layerFEState)) {
        // ALOGE("BAT %p return2", layerFE.get());
        return;
    }

    // handle hidden surfaces by setting the visible region to empty
    if (CC_UNLIKELY(!layerFEState->isVisible)) {
        // ALOGE("BAT %p return3", layerFE.get());
        return;
    }
    ...
    if (visibleRegion.isEmpty()) {
        // ALOGE("BAT %p return4", layerFE.get());
        return;
    }
    ...
    if (visibleRegion.isEmpty()) {
        // ALOGE("BAT %p return5", layerFE.get());
        return;
    }
    ...
    if (drawRegion.isEmpty()) {
        // ALOGE("BAT %p return6", layerFE.get());
        return;
    }
    ...
    // ALOGE("BAT create outlayer from %p", layerFE.get());
...
void Output::updateLayerStateFromFE(const CompositionRefreshArgs& args) const {
    // ALOGE("BAT updateLayerStateFromFE begin");
    for (auto* layer : getOutputLayersOrderedByZ()) {
        // ALOGE("BAT updateLayerStateFromFE layer %p", &(layer->getLayerFE()));
        layer->getLayerFE().prepareCompositionState(
                args.updatingGeometryThisFrame ? LayerFE::StateSubset::GeometryAndContent
                                               : LayerFE::StateSubset::Content);
    }
    // ALOGE("BAT updateLayerStateFromFE end");
}
...
void Output::finishFrame(const compositionengine::CompositionRefreshArgs& refreshArgs) {
    ...
    if (!getState().isEnabled) {
        // ALOGV("BAT getState().isEnabled is false");
        return;
    }
...
std::optional<base::unique_fd> Output::composeSurfaces(
    ...
    ALOGV("BAT hasClientComposition");
    ...
    if (mClientCompositionRequestCache) {
        if (mClientCompositionRequestCache->exists(buf->getId(), clientCompositionDisplay,
            ...
            setExpensiveRenderingExpected(false);
            ALOGE("BAT mClientCompositionRequestCache->exists is true return");
            return readyFence;
        }
...
std::vector<LayerFE::LayerSettings> Output::generateClientCompositionRequests(
    ...
    // ALOGE("BAT Rendering client layers");
    ...
    for (auto* layer : getOutputLayersOrderedByZ()) {
        ...
        if (clip.isEmpty()) {
            ...
            // ALOGE("BAT Skipping for empty clip %p layerFE:%p", layer, &layerFE);
            firstLayer = false;
            continue;
        }
        ...
        // ALOGE("BAT clientComposition:%d clearClientComposition:%d layerFE:%p", clientComposition, clearClientComposition, &layerFE);
