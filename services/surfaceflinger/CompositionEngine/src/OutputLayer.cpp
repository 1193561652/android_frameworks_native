...
void OutputLayer::updateCompositionState(
    ...
    if (includeGeometry) {
        ...
        if ((layerFEState->isSecure && !outputState.isSecure) ||
            ...
            // ALOGE("BAT set forceClientComposition true layerFEState->isSecure:%d outputState.isSecure:%d state.bufferTransform:%d", 
            //     layerFEState->isSecure, outputState.isSecure, state.bufferTransform);
    ...
    if (layerFEState->forceClientComposition || !profile.isDataspaceSupported(state.dataspace) ||
        ...
        ALOGE("BAT %p %p set forceClientComposition true layerFEState->forceClientComposition:%d isDataspaceSupported:%d forceClientComposition:%d", 
             this, &(getLayerFE()), layerFEState->forceClientComposition, profile.isDataspaceSupported(state.dataspace), forceClientComposition);
        state.forceClientComposition = true;
    }
}
...
void OutputLayer::writeCompositionTypeToHWC(HWC2::Layer* hwcLayer,
    ...
    if (outputDependentState.forceClientComposition) {
        ...
        ALOGE("BAT outputDependentState.forceClientComposition %p is true set hal::Composition::CLIENT", this);
    } else {
        // ALOGE("BAT outputDependentState.forceClientComposition %p is false", this);
    }
